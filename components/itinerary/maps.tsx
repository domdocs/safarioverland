/**
 * Editorial SVG maps for the itinerary preview.
 *
 * Ported verbatim from handoff/design_handoff_itinerary/source-prototype/src/maps.jsx
 * — the stylised equirectangular projection, hand-traced country
 * polygons, dashed route line, and stop markers are all intentional
 * design choices, not survey-grade cartography. Do not "improve" them
 * without sign-off.
 *
 * Pure server components — no client interactivity.
 */

import { toRoman } from "@/lib/itineraries/types"

// ── projection (stylised) ───────────────────────────────────────────────
// lon 10°E → 36°E, lat -36°S → -8°S, mapped into an 800×900 viewBox.
const X0 = 10
const X1 = 36
const Y0 = -36
const Y1 = -8
...
function Compass({ x = 70, y = 110 }: { x?: number; y?: number }) {
  return (
    <g transform={`translate(${x} ${y})`} fontFamily="var(--display)" fontSize="11">
      ...
      <text x="0" y="-40" textAnchor="middle" fontStyle="italic">
        N
      </text>
      <text x="0" y="48" textAnchor="middle" fontStyle="italic" opacity="0.6">
        S
      </text>
    </g>
  )
...
function Scale({ x = 540, y = 850 }: { x?: number; y?: number }) {
  return (
    <g
    ...
    >
      ...
      <text x="0" y="18" textAnchor="middle" opacity="0.7">
        0
      </text>
      <text x="90" y="18" textAnchor="middle" opacity="0.7">
        300
      </text>
      <text x="180" y="18" textAnchor="middle" opacity="0.7">
        600 KM
      </text>
    </g>
  )
...
type OverviewMapProps = {
  stops: MapStop[]
  /** Caption text — defaults to "The Route — Southern Africa". */
  caption?: string
  /** Sub-caption, e.g. "14 — 24 SEPTEMBER 2026 · ≈ 2,500 KM". */
  subCaption?: string
}

export function OverviewMap({
  stops,
  caption = "The Route — Southern Africa",
  subCaption,
}: OverviewMapProps) {
  const valid = stops
    .filter((s) => Number.isFinite(s.lat) && Number.isFinite(s.lon))
    .map((s) => ({ ...s, ...projectCoords(s.lon, s.lat) }))
  ...
  return (
    <svg viewBox={`0 0 ${VW} ${VH}`} style={{ color: "var(--ink)" }}>
      ...
      {valid.map((s, i) => {
        ...
        // Alternate label sides to avoid collision — east of the line on
        // even stops, west on odd. Cheap heuristic; works for typical 2-5
        // stop trips.
        const right = i % 2 === 0
...
export function TransitMap({ from, to }: TransitMapProps) {
  if (
  ...
  const f = projectCoords(from.lon, from.lat)
  const t = projectCoords(to.lon, to.lat)

  const padX = Math.max(60, Math.abs(t.x - f.x) * 0.6)
  const padY = Math.max(60, Math.abs(t.y - f.y) * 0.6)
  const minX = Math.min(f.x, t.x) - padX
  const minY = Math.min(f.y, t.y) - padY
  const w = Math.abs(t.x - f.x) + padX * 2
  const h = Math.abs(t.y - f.y) + padY * 2

  const visibleCountries = COUNTRIES.filter(
    (c) =>
      c.labelAt[0] > minX - 40 &&
      c.labelAt[0] < minX + w + 40 &&
      c.labelAt[1] > minY - 40 &&
      c.labelAt[1] < minY + h + 40,
  )
